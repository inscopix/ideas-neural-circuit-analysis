"""Combine and compare correlation data across groups and conditions."""
...
def average_correlations(data: List[dict]) -> pd.DataFrame:
    """Calculate average positive and negative correlations per state.

    Computes averages across multiple recordings, excluding diagonal elements.
        ...
        with outputs.register(raise_missing_file=False) as output_data:
            ...
            for output_name in output_names:
                output_file = (
                    ...
                    .register_preview(
                        "average_correlation_distribution"
                        f"{config.OUTPUT_PREVIEW_SVG_FILE_EXTENSION}",
                        caption=(
                            "Figure displaying box and whisker plots of average positive and "
                            "negative correlations across experimental states. The plot shows "
                    ...
                    .register_preview(
                        f"{statistic.lower()}_correlation_state_lmm"
                        f"{config.OUTPUT_PREVIEW_SVG_FILE_EXTENSION}",
                        caption=(
                            f"Figure presenting linear mixed model (LMM) analysis "
                    ...
                    .register_preview(
                        f"{statistic.lower()}_correlation_group_{anova_type}"
                        f"{config.OUTPUT_PREVIEW_SVG_FILE_EXTENSION}",
                        caption=(
                            f"Figure displaying analysis of variance (ANOVA) "
