"""Utilities for combining and comparing peri-event data across epochs."""
...
from utils.utils import remove_unsupported_characters, compute_sampling_rate
...
def strtobool(value: str) -> bool:
    """Convert common truthy/falsey strings to a boolean value."""
    # this func has been deprecated in python 3.13
    # implementation is easy to copy:
...
def extract_modulation_group_data(
    ...
    for traces_df, stats_df, stats_filename in zip(
        ...
        if total_num_cells == 0:
            logger.warning(
                "No cells detected for epoch "
                f"'{epoch_name}' in file '{os.path.basename(stats_filename)}', "
                "the corresponding modulation data cannot be calculated"
            )
            continue
...
def _compare_data(
    ...
    if num_epochs < 2:
        logger.warning(
            "A minimum of 2 epochs are needed to run ANOVA. "
            "The ANOVA output file will not be generated."
        )
        aov = None
    else:
...
def combine_peri_event_data(
    ...
    for i in range(len(stats_files)):
        ...
        # drop (population, up_modulated, down_modulated, non_modulated) to retain per-cell data
        # only
...
def _validate_epoch_names_within_group(
    ...
    for traces_file, stats_file in zip(group_traces_files, group_stats_files):
        ...
        if traces_epoch_names != stats_epoch_names:
            raise IdeasError(
                "The traces and statistics files do not contain the same epochs: "
                f"{', '.join(traces_epoch_names)} vs {', '.join(stats_epoch_names)}",
            )

        # log warnings when user-specified epoch names don't match those in the input files
        num_epochs_not_found = 0
        for epoch_name in epoch_names:
            if epoch_name not in traces_epoch_names:
                logger.warning(
                    "No data found for epoch "
                    f"'{epoch_name}' in file '{os.path.basename(traces_file)}'"
                )
...
def combine_compare_peri_event_data_across_epochs(
    ...
    :param average_method: indicates whether averaging should be done across all
                           observations or over each recording
    :param tolerance: Maximum time shift in seconds between the time windows of the input traces
        files
    ...
    if len(group1_traces_files) > 1:
        ...
        group1_data, group1_md = combine_peri_event_data(
            ...
            population_activity_plot_limits=population_activity_plot_limits,
            activity_heatmap_color_limits=activity_heatmap_color_limits,
            activity_by_modulation_plot_limits=plot_limits,
        )
    ...
    if len(group1_traces_files) > 1 and len(group2_traces_files) > 1:
        ...
        if len(common_epochs) == 0:
            ...
            if sorted(group1_epoch_names) != sorted(group2_epoch_names):
                logger.warning(
                    "The epochs in the first group "
                    f"({', '.join(group1_epoch_names)}) do not match the epochs in the "
                    "second group "
                    f"({', '.join(group2_epoch_names)}). Only the epochs common to both "
                    f"groups ({', '.join(common_epochs)}) will be compared."
                )
        ...
        with outputs.register(raise_missing_file=False) as output_data:
            for group_name in [group1_name, group2_name]:
                ...
                output_file = output_data.register_file(
                    ...
                    caption=(
                        "Comparison of event-aligned average population activity across the epochs."
                    ),
                )
                for epoch_name in epoch_names:
                    output_file.register_preview(
                        f"event_aligned_population_activity_{group_name}_{epoch_name}"
                ...
                for epoch_name in epoch_names:
                    output_file.register_preview(
                        f"event_aligned_activity_by_modulation_{group_name}_{epoch_name}"
                        f"{config.OUTPUT_PREVIEW_SVG_FILE_EXTENSION}",
                        caption=(
                ...
                for epoch_name in epoch_names:
                    output_file.register_preview(
                        f"fraction_of_modulated_neurons_{group_name}_{epoch_name}"
                        f"{config.OUTPUT_PREVIEW_SVG_FILE_EXTENSION}",
                        caption=(
