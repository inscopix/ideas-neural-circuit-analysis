        ...
        for file_path in file_list:
            ...
            if "correlation" in file_type.lower():
                if file_path.suffix.lower() not in [".csv", ".h5"]:
                    raise IdeasError(
                        f"Invalid correlation file format: {file_path}. Must be .csv or .h5 file."
                    )
...
def _compute_correlation_y_limits(
    ...
    def _range_from_series(
        ...
        if valid_data.empty:
            logger.info(
                f"No valid {label} correlation values found - using auto-scaling for y-limits"
            )
            return None
...
def _prepare_modulation_distribution_data(
    ...
    def _select_metadata_value(
        ...
        if conflicts:
            ...
            logger.warning(
                "Conflicting '%s' values found for modulation grouping '%s': %s. Using '%s'.",
                metadata_column,
                label,
                conflict_list,
                representative,
