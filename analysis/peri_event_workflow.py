"""Workflow helpers for peri-event analyses and plotting."""
...
def extract_mean_event_windows_per_event_shuffle(
    ...
    """Extract mean event windows for each shuffle of event times.

    Given shuffled event times, splice out window around each event and compute the mean
    event window per shuffle.
...
def generate_post_minus_pre_shuffled_dist(
    ...
    """Generate a post minus pre distribution from shuffled event windows.

    The distribution is computed using event windows collected from shuffled event indices.
...
def compute_statistical_metrics(
    ...
    """Compute metrics for every cell by comparing post minus pre value to null.
