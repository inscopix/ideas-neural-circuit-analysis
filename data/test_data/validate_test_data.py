...
class TestDataValidator:
    ...
    def _validate_activity_file(
        ...
        if len(df) != expected_rows:
            self.errors.append(
                f"{subject}: Activity file has {len(df)} rows, expected {expected_rows}"
            )
        else:
    ...
    def _validate_correlation_file(
        ...
        if len(df) != expected_rows:
            self.errors.append(
                f"{subject}: Correlation file has {len(df)} rows, expected {expected_rows}"
            )
        else:
        ...
        if is_sparse_no_events:
            ...
            if has_event_cols:
                ...
                print(
                    "  SUCCESS: Event correlation columns properly removed (sparse scenario)"
                )
        elif has_event_cols:
            ...
            if is_sparse_partial:
                ...
                if nan_count > 0:
                    ...
                    self.warnings.append(
                        f"{subject}: Partial events scenario should have some NaN values"
                    )
        else:
    ...
    def _validate_modulation_file(
        ...
        if len(df) != expected_rows:
            self.errors.append(
                f"{subject}: Modulation file has {len(df)} rows, expected {expected_rows}"
            )
        else:
        ...
        if is_sparse_no_events:
            if event_mod_cols:
                ...
                print(
                    "  SUCCESS: Event modulation columns properly removed (sparse scenario)"
                )
        elif event_mod_cols:
    ...
    def _validate_cross_file_consistency(
        ...
        if activity_cells != self.N_CELLS:
            self.warnings.append(
                f"{subject}: Expected {self.N_CELLS} cells, found {activity_cells}"
            )
        ...
        if activity_combos != correlation_combos:
            self.warnings.append(
                f"{subject}: State-epoch combinations differ between activity and correlation files"
            )
