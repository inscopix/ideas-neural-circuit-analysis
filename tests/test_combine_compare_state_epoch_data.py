...
try:
    import ideas.outputs  # type: ignore
...
try:
    import ideas.plots  # type: ignore  # noqa: F401
...
class TestAddModulationCountColumns:
    ...
    def test_add_counts_with_multiple_states_and_epochs(self):
        ...
        awake_stim = result[(result["state"] == "awake") & (result["epoch"] == "stim")]
        ...
        sleep_stim = result[(result["state"] == "sleep") & (result["epoch"] == "stim")]
        ...
        df = pd.DataFrame(
            {
                ...
                "trace_modulation": [
                    ...
                    np.nan,
                    0,
                ],  # 1 up, 1 down, 1 NaN, 1 non-modulated
            }
        )
        ...
        df = pd.DataFrame(
            {
                ...
                "trace_modulation": [
                    ...
                    -1,
                    1,
                ],  # subject1: 2 up, subject2: 1 up, 2 down
            }
        )
...
class TestDetectMeasureColumnForModulationCounts:
    ...
    def test_detect_trace_up_modulated_counts(self):
        ...
        for data_type in ["trace_up_modulated_counts", "trace_up_modulation_counts"]:
            ...
            assert (
                result == "trace_up_modulation_number"
    ...
    def test_detect_trace_down_modulated_counts(self):
        ...
        for data_type in [
            ...
            "trace_down_modulation_counts",
        ]:
            result = _detect_measure_column(df, data_type)
            assert (
                result == "trace_down_modulation_number"
            ), f"Failed for data_type: {data_type}"
    ...
    def test_detect_event_up_modulated_counts(self):
        ...
        for data_type in ["event_up_modulated_counts", "event_up_modulation_counts"]:
            result = _detect_measure_column(df, data_type)
            assert (
                result == "event_up_modulation_number"
            ), f"Failed for data_type: {data_type}"
    ...
    def test_detect_event_down_modulated_counts(self):
        ...
        for data_type in [
            "event_down_modulated_counts",
            "event_down_modulation_counts",
        ]:
            result = _detect_measure_column(df, data_type)
            assert (
                result == "event_down_modulation_number"
            ), f"Failed for data_type: {data_type}"

    def test_detect_prefers_trace_when_both_available_up(self):
        """Prefer trace when both trace and event up-modulated counts are available."""
        df = pd.DataFrame(
            {
                "trace_up_modulation_number": [5, 3, 7],
    ...
    def test_detect_prefers_trace_when_both_available_down(self):
        """Prefer trace when both trace and event down-modulated counts are available."""
        df = pd.DataFrame(
            {
    ...
    def test_detect_falls_back_to_event_up(self):
        ...
        df = pd.DataFrame({"event_up_modulation_number": [5, 3, 7]})
    ...
    def test_detect_falls_back_to_event_down(self):
        ...
        df = pd.DataFrame({"event_down_modulation_number": [5, 3, 7]})
    ...
    def test_detect_raises_when_column_not_found(self):
        ...
        df = pd.DataFrame({"some_other_column": [5, 3, 7]})
    ...
    def test_detect_case_insensitive_matching(self):
        ...
        df = pd.DataFrame({"trace_up_modulation_number": [5, 3, 7]})
...
class TestAddModulationCountsToStatisticalOutput:
    ...
    def test_placeholder_state_values_are_ignored(self):
        ...
        assert (
            result.loc[
                result["epoch"] == "baseline", "trace_up_modulation_number"
            ].iloc[0]
            == 2
        )
